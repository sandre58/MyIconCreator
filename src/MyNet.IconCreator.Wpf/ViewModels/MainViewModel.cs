...
internal sealed class MainViewModel : ObservableObject
{
    ...
    public MainViewModel(IBusyService busyService)
    {
        ...
        const string fontFamilyKey = FontFamilyKey;
    ...
    private async Task SaveImageAsync(object x)
    {
        ...
        if (result.IsTrue())
            await BusyService.WaitIndeterminateAsync(() => UI.Threading.Scheduler.UiOrCurrent.Schedule(() => XamlToImageFileService.SaveImage(fe, filename))).ConfigureAwait(false);
    }
